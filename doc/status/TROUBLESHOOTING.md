# Resolução de Problemas Google Calendar MCP

Este documento serve como um índice para os problemas comuns e suas soluções encontradas durante o desenvolvimento e uso do servidor MCP do Google Calendar.

- [Problemas Iniciais](troubleshooting/initial_problems.md)
- [Passos de Diagnóstico](troubleshooting/diagnostic_steps.md)
- [Correções Aplicadas](troubleshooting/applied_corrections.md)
- [Testes Realizados](troubleshooting/tests_performed.md)
- [Próximos Passos](troubleshooting/next_steps.md)

## Documentação Relacionada

### Documentação Interna

- [Visão Geral](../guides/overview.md)
- [Instalação](../guides/installation.md)
- [Arquitetura](../guides/architecture.md)
- [Uso](../guides/usage.md)
- [Desenvolvimento Futuro](../future.md)
- [Sumário da Documentação](../README.md)
